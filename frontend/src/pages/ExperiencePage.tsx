...
export function ExperiencePage() {
  ...
  const validateForm = () => {
    ...
    if (!formData.cargo_titulo.trim()) newErrors.cargo_titulo = 'El cargo/título es obligatorio';
    if (!formData.institucion_empresa.trim()) newErrors.institucion_empresa = 'La institución/empresa es obligatoria';
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('¿Estás seguro de eliminar esta experiencia? Esta acción no se puede deshacer.')) {
      try {
  ...
  return (
    <div>
      ...
      <Card className="mb-8">
        ...
        {loading ? (
           <p className="text-center py-6 opacity-60">Cargando Experiencia Laboral...</p>
        ) : laboralExperiences.length === 0 ? (
          <div className="text-center py-12">
            <div className="w-16 h-16 rounded-full bg-muted flex items-center justify-center mx-auto mb-4">
              <Briefcase className="w-8 h-8 opacity-30" />
            </div>
            <p className="opacity-60 mb-4">No tienes experiencia laboral registrada</p>
      ...
      <Card>
        ...
        {loading ? (
           <p className="text-center py-6 opacity-60">Cargando Formación Académica...</p>
      ...
      <Modal
        ...
        <form className="space-y-6">
          <Select
            ...
            name="tipo"
            value={formData.tipo}
            onChange={(e) => setFormData({ ...formData, tipo: e.target.value as 'laboral' | 'academica' })}
            options={[{ value: 'laboral', label: 'Experiencia Laboral' }, { value: 'academica', label: 'Formación Académica' }]}
            required />

          <Input
            label={formData.tipo === 'laboral' ? 'Cargo' : 'Título'}
            name="cargo_titulo"
            placeholder={formData.tipo === 'laboral' ? 'Ej: Desarrollador Full Stack' : 'Ej: Ingeniería de Sistemas'}
          ...
          <Input
            ...
            placeholder={formData.tipo === 'laboral' ? 'Ej: Tech Company' : 'Ej: Universidad Mayor de San Simón'}
          ...
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="actual" className="text-sm ">
              {formData.tipo === 'laboral' ? 'Actualmente trabajo aquí' : 'Actualmente estudio aquí'}
            </label>
          </div>
        </form>
      </Modal>
