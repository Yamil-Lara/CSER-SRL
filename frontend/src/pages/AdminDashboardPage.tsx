...
function EstadoToggle({ activo }: { activo: number }) {
    ...
    return (
        <div
            className={`relative inline-flex items-center w-10 h-6 rounded-full transition-colors duration-200 ${on ? 'bg-emerald-500' : 'bg-gray-300 dark:bg-gray-600'
                }`}
        >
            <span
                className={`absolute left-1 w-4 h-4 bg-white rounded-full shadow transition-transform duration-200 ${on ? 'translate-x-4' : 'translate-x-0'
                    }`}
            />
        </div>
    );
