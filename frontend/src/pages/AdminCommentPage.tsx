...
export default function AdminCommentModeration() {
  ...
  const fetchData = async () => {
      ...
      try {
          ...
          const res = await api.get('/gestion/comentarios/pendientes');
                      ...
                      <Card key={c.id} className="overflow-hidden border-muted shadow-md hover:shadow-lg transition-shadow p-6">
                          <div className="flex flex-col md:flex-row gap-6">
                              ...
                              <div className="flex md:flex-col gap-2 justify-center border-t md:border-t-0 md:border-l border-muted pt-4 md:pt-0 md:pl-6 min-w-[140px]">
                                  {c.aprobado !== 1 && (
                                      <button onClick={() => handleUpdateStatus(c.id, 1)} className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-all text-xs font-bold">
                                          <CheckCircle className="w-4 h-4" /> Aprobar
                                      </button>
                                  )}
                                  {c.aprobado !== 2 && (
                                      <button onClick={() => handleUpdateStatus(c.id, 2)} className="flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-all text-xs font-bold">
